...
var submitScore = async (event) => {
  ...
  if (score > 1e3) {
    const connection = await dynamoClient_default.send(
    ...
    if (connectionId) {
      await sendWebSocketMessage(connectionId, {
        type: "notification",
        message: "\u{1F389} Congrats! You scored over 1000!"
      });
    }
  }
  return {
...
var sendWebSocketMessage = async (connectionId, message) => {
  const client2 = new import_client_apigatewaymanagementapi.ApiGatewayManagementApiClient({
